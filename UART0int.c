...
void static copyHardwareToSoftware(void){
  ...
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (RxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART0_DR_R;
    RxFifo_Put(letter);
    OS_Signal(&RxDataAvailable); //signal in interrupt is fine (no loop)
  }
}
...
char UART_InChar(void){
  ...
  while(RxFifo_Get(&letter) == FIFOFAIL){}; // this might not be needed
  return(letter);
}
...
void UART_OutChar(char data){
  OS_Wait(&TxRoomLeft);
  while(TxFifo_Put(data) == FIFOFAIL){}; // this might not be needed
